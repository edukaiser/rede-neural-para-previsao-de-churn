...
* **Deploy**: API de alta performance desenvolvida com **FastAPI**, conteinerizada com Docker e hospedada em nuvem.

## 🚀 Deploy e Acesso à API

A aplicação está em produção utilizando **Azure Container Instances (ACI)**, servindo predições em tempo real através de um endpoint público.

* **Link do Repositório (GitHub)**: [https://github.com/edukaiser/rede-neural-para-previsao-de-churn]
* **Swagger UI (Documentação da API)**: [http://previsao-churn-eduardo.eastus2.azurecontainer.io:8000/docs]
...
![Comparação de Baselines](img/comparacao.png)
...
Para provisionar a API de inferência localmente:
```bash
